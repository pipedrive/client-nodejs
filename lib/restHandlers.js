(function() {
	...
	RestHandlers.prototype.listItems = function(object, params, callback) {
		var self = this;
		var paramsToSupply = _.extend({}, _.isObject(params) ? params : {}, this.options.apiToken ? { api_token: this.options.apiToken } : {});
		var dataObject = object === 'authorizations' ? { multipart: false, data: paramsToSupply } : { query: qs.stringify(paramsToSupply), oauth_token: this.options.apiToken};

		var requestUrl = apiUrl(object, this.options, false);
		var req = rest[object === 'authorizations' ? 'post' : 'get'](requestUrl, dataObject);
	...
	RestHandlers.prototype.findItems = function(object, params, callback) {
		var self = this;
		var paramsToSupply = _.extend({}, _.isObject(params) ? params : {}, this.options.apiToken ? { api_token: this.options.apiToken } : {});
		var dataObject = { query: qs.stringify(paramsToSupply), oauth_token: this.options.apiToken };
		var req = rest.get(apiUrl(object + '/find', this.options, false), dataObject);
		req.on('complete', function(data, res) {
			self.genericResponse('GET', object, data, callback, req, res);
		});
	...
	RestHandlers.prototype.removeManyItems = function(itemIds, object, params, callback) {
		...
		var self = this;
		var req = rest.del(apiUrl(object, this.options, true), { json: true, multipart: false, data: (_.isObject(params) && !_.isFunction(params) ? params : { ids: itemIds }), query: qs.stringify({ids: ids}), oauth_token: this.options.apiToken });
	...
	RestHandlers.prototype.mergeItem = function(whichId, withId, object, callback) {
		...
		var self = this,
			req = rest.put(apiUrl(object + '/' + whichId + '/merge', this.options, true), { json: true, multipart: false, data: { merge_with_id: withId }, oauth_token: this.options.apiToken });
