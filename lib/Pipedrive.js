...
(function () {
	...
	exports.OAuthAuthenticate = function (client_id, redirect_uri, res) {
	...
	exports.OAuthAuthenticateCallback = function (req, res, redirect_uri, clientId, clientSecret) {
	    ...
	    return new Promise(function (resolve, reject) {
			var req = rest.post(authOptions.url,
				{
					json: authOptions.json,
					multipart: false,
					data: authOptions.form,
					basic_auth: authOptions.headers['Authorization']
				});

			req.on('complete', function(data, res) {
				if (!data) {
					log('OAuthAuthenticateCallback.error handling: ' + res);
					return reject(res)
				}
				var body = JSON.parse(data);
				log('access_token: ' + body.access_token);
				resolve(body)
			});
	    })
	};

	exports.OAuthRefreshToken = function (req, res, refresh_token, clientId, clientSecret) {
	    var refreshOptions = {
	        url: 'https://oauth.pipedrive.com/oauth/token',
            form: {
	    ...
	    return new Promise(function (resolve, reject) {
			var req = rest.post(refreshOptions.url,
				{
					json: refreshOptions.json,
					multipart: false,
					data: refreshOptions.form,
					basic_auth: refreshOptions.headers['Authorization']
				});

			req.on('complete', function(data, res) {
				if (!data) {
					log('OAuthRefreshToken.error handling: ' + res);
					return reject(res)
				}
				var body = JSON.parse(data);
				log('new access_token: ' + body.access_token);
				log('new refresh_token: ' + body.refresh_token);
				resolve(body)
			});
	    })
	};
