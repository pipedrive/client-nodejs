	...
	exports.OAuthAuthenticateCallback = function (req, res, redirect_uri, clientId, clientSecret) {
	    ...
	    return new Promise(function (resolve, reject) {
			...
			req.on('complete', function(data, res) {
				...
				log('DATA2: ' + data);
				log('DATA2.a: ' + data.access_token);
				log('DATA2.b: ' + data['access_token']);
	...
	exports.OAuthRefreshToken = function (req, res, refresh_token, clientId, clientSecret) {
	    ...
	    return new Promise(function (resolve, reject) {
	        request.post(refreshOptions, function (error, response, body) {
	            if (error) {
	                log('OAuthAuthenticateCallback.error handling: ' + error);
	                return reject(error)
	            }
	            var accessToken = body.access_token;
	            var refreshToken = body.refresh_token;
	            log('new access_token: ' + accessToken);
	            log('new refresh_token: ' + refreshToken);
	            resolve(body)
	        })
	    })
	};
