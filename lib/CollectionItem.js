(function() {
	...
	function CollectionItem(kind, data, itemId, options, undefinedProperty) {
		...
		this.save = function(callback) {
			if (!_.isFunction(callback)) {
				return new Promise(function (resolve, reject) {
					this.saveInternal(function (err, items) {
						if(!err) {
							resolve(items);
						} else {
							reject(err);
			...
			this.saveInternal(callback);

			changedData = {};
			return currentItem;
		};

		this.saveInternal = function(callback) {
			restHandlers.editItem(data.id, kind, changedData, function (error, _data, additionalData, req, res) {
				var collectionItems = wrapCollectionItems([_data], kind, options);
		...
		this.remove = function(callback) {
			...
			return (!_.isFunction(callback)) ? asPromise(method, [data.id, kind, {}, null]) : method.apply(restHandlers, [data.id, kind, {}, callback]);
		};
		...
		this.merge = function (withId, callback) {
			if (!_.isFunction(callback)) {
				return new Promise(function (resolve, reject) {
					this.mergeInternal(withId, function (err, items) {
						if(!err) {
							resolve(items);
						} else {
							reject(err);
			...
			return this.mergeInternal(withId, callback);
		};

		this.mergeInternal = function(withId, callback) {
		...
		this.duplicate = function (callback) {
			if (!_.isFunction(callback)) {
				return new Promise(function (resolve, reject) {
					this.duplicateInternal(function (err, items) {
						if(!err) {
							resolve(items);
						} else {
							reject(err);
			...
			return this.duplicateInternal(callback);
		};

		this.duplicateInternal = function (callback) {
		...
		function asPromise(method, args) {
			return new Promise(function (resolve, reject) {
				args[args.length - 1] = function (err, _data) {
					if (!err && _data !== null) {
						resolve(_data);
					} else {
						reject(err);
