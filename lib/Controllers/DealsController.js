...
class DealsController {
     ...
     * @param {boolean} input['exact_match'] (optional) When enabled, only full exact matches against the given term are returned.
     * @param {int} input['person_id'] (optional) Will filter Deals by the provided Person ID.
     * @param {int} input['organization_id'] (optional) Will filter Deals by the provided Organization ID.
     * @param {string} input['status'] (optional) Will filter Deals by the provided specific status.
     * @param {string} input['include_fields'] (optional) Supports including optional fields in the results which are not provided by default.
     ...
     * @param  {array} input Array with all options for search.
     * @param {string} input['term'] Search term to look for.
     * @param {string} input['item_types'] (optional) A comma-separated string array. The type of items to perform the search from.
     * @param {string} input['fields'] (optional) A comma-separated string array. The fields to perform the search from.
     * @param {boolean} input['search_for_related_items'] (optional) Up to 100 newest related Leads and 100 newest related Deals for each found Person and Organization and up to 100 newest related Persons for each found Organization.
     * @param {boolean} input['exact_match'] (optional) When enabled, only full exact matches against the given term are returned. It is not case sensitive.
     * @param {string} input['include_fields'] (optional) A comma-separated string array. Supports including optional fields in the results which are not provided by default.
     * @param {int} input['start'] (optional) Pagination start. Note that the pagination is based on main results and does not include related items when using search_for_related_items parameter.
     * @param {int} input['limit'] (optional) Items shown per page.
     *
     * @callback    The callback function that returns response from the API call
     *
    ...
    static searchItem(input, callback) {
        ...
        if (_oAuthManager.isTokenSet()) {
            return DealsController.searchItemAction(_queryUrl, _callback, input);
        }
        return _oAuthManager.authorize().then(
            () => DealsController.searchItemAction(_queryUrl, _callback, input),
            (err) => {
                _callback(err, null, null);
                return Promise.reject(err);
            });
     ...
     * @param {string} input['field_type'] (required) The type of the field to perform the search from.
     * @param {string} input['field_key'] (required) The key of the field to search from.
     * @param {boolean} input['exact_match'] (optional) When enabled, only full exact matches against the given term are returned. It is not case sensitive.
     * @param {boolean} input['return_item_ids'] (optional) Whether to return the IDs of the matching items or not.
     * @param {int} input['start'] (optional) Pagination start. Note that the pagination is based on main results and does not include related items when using search_for_related_items parameter.
     * @param {int} input['limit'] (optional) Items shown per page.
     *
     * @callback    The callback function that returns response from the API call
    ...
    static searchItemByField(input, callback) {
        ...
        if (_oAuthManager.isTokenSet()) {
            return DealsController.searchItemByFieldAction(_queryUrl, _callback, input);
        }
        return _oAuthManager.authorize().then(
            () => DealsController.searchItemByFieldAction(_queryUrl, _callback, input),
            (err) => {
                _callback(err, null, null);
                return Promise.reject(err);
            });
