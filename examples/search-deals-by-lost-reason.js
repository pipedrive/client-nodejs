...
pipedrive.SearchResults.field({ term: process.argv[3], exact_match: false, field_type: 'dealField', field_key: 'lost_reason', return_item_ids: true, limit: 10 }, function(dealsListErr, dealsList) {
	...
	_.each(dealsList, function(deal) {
		pipedrive.Deals.get(deal.id, function(err, fullDealData) {
			...
			deal.getActivities(function(err, activities) {
				...
				console.log('* ' + fullDealData.get('title') + ' (Lost reason: ' + fullDealData.lost_reason + ') was worth ' + fullDealData.get('value') + ' ' + fullDealData.get('currency') + ', has ' + activities.length + ' activit' + (activities.length != 1 ? 'ies' : 'y'));
			});
		});
	});
});
